...
{
    // 容量 = heads x cylinders x sectors x 512
	geo->heads = 2;     // 磁头多少面
	geo->sectors = 32;  // 柱面 多少环
	geo->cylinders = RAMBLOCK_SIZE/2/32/512; // 一环多少扇区 1024*1024/2/512/32
	return 0;
}
static struct block_device_operations ramdisk_fops = {
	.owner	= THIS_MODULE,
    .getgeo = ramblock_getgeo, // 获取几何信息，分区时提供柱面数cylinders
};
