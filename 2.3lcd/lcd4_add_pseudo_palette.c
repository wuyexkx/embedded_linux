			     ...
			     unsigned int transp, struct fb_info *info)
{
    ...
    if (regno > 16) 
        return 1;
    // 用red green blue三原色构造val值
    val  = chan_to_field(red, &info->var.red);
    val |= chan_to_field(green, &info->var.green);
    val |= chan_to_field(blue, &info->var.blue);
	// 将计算的值付给对应号的调色板
    // ((u32 *)(info->pseudo_palette))[regno] = val;
    pseudo_pal[regno] = val; // 本地的假调色板
	return 0;
}
...
static int lcd_init(void)
{
    ...
    // 2.4其他设置
    s3c_lcd_fbinfo->pseudo_palette  = pseudo_pal; // 假调色板16种颜色 Fake palette of 16 colors 
...
{
    unregister_framebuffer(s3c_lcd_fbinfo);

    dma_free_writecombine(NULL, \
        s3c_lcd_fbinfo->fix.smem_len,\
        s3c_lcd_fbinfo->screen_base, \
        s3c_lcd_fbinfo->fix.smem_start);

    iounmap(lcd_regs); 
    iounmap(gpgcon);
    iounmap(gpdcon);
    iounmap(gpccon); 
    framebuffer_release(s3c_lcd_fbinfo);
}
