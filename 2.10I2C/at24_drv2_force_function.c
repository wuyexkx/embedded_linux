...
static unsigned short force_addr[] = { ANY_I2C_BUS, 0x60, I2C_CLIENT_END }; // 板子上i2c设备不是这个地址
static unsigned short* forces[] = { force_addr, NULL }; 

static struct i2c_client_address_data addr_data = {
    .normal_i2c = ignore, // 一般要发出S信号和设备地址并得到ACK才能确定设备存在 才会function，这里设备不存在也会强制执行function
    .probe      = ignore,
    .ignore     = ignore,
    .forces     = forces, // 强制认为这个设备存在 当前不能用但是以后会用到 还是希望调用那个function
};
// 那个function
